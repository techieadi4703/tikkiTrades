...
export default function WatchlistClient({ initialData }) {
  ...
  return (
    <div className="p-6">
      ...
      <div className="rounded-xl border border-gray-700 overflow-hidden">
        <div className="grid bg-gray-900 px-4 py-3 text-sm text-gray-300"
          style={{ gridTemplateColumns: "40px 1.5fr 1fr 1fr 1fr 1fr 1fr 1.5fr" }}
        >
          <div>★</div>
        ...
        {data.map(item => {
          ...
          return (
            ...
            >
              <div>⭐</div>
              <Link href={`/stocks/${item.symbol}`} className="font-bold hover:text-yellow-400">
                {item.company}
              </Link>
              <div>{item.symbol}</div>
